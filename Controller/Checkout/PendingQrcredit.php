...
class PendingQrcredit extends \GBPrimePay\Payments\Controller\Checkout
{
    /**
     * Dispatch request
     *
      ...
      try {
          ...
          if (($_orderId == $_getEntityId ) && ($_getIncrementId == $_getOrderByEntityId )) {
                ...
                if($ordercompletestatus != 0){
                    ...
                    if ($this->_config->getEnvironment() === 'prelive') {
                        ...
                        $url = Constant::URL_QRCREDIT_LIVE;
                ...
                $itemresponseurl = $this->_config->getresponseUrl('response_qrcredit');
                $itembackgroundurl = $this->_config->getresponseUrl('background_qrcredit');
        ...
        return $result;
          ...
          if ($this->_config->getCanDebug()) {
              $this->gbprimepayLogger->addDebug("PendingQrcredit error //" . $exception->getMessage());
          }
          $this->cancelOrder();
