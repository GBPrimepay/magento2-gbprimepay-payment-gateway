...
define(
    [
        ...
        'use strict';
                ...
                var generator = setInterval(function () {
                    if ($("input[name='payment[transaction_id]']").length > 0) {
                      ...
                      if ((hash == "#payment") && (selected == "gbprimepay_qrwechat")) {
        ...
        return Component.extend({
            defaults: {
                template: 'GBPrimePay_Payments/payment/gbprimepay_qrwechat',
            ...
            getCode: function () {
                return 'gbprimepay_qrwechat';
            },
            validate: function () {
                return true;
            },
            getInstructionQrwechat: function () {
                return window.gbprimepay.instructionQrwechat;
            },
            getLogoQrwechat: function () {
                return window.gbprimepay.logoQrwechat;
            },
            getTitleQrwechat: function () {
                return window.gbprimepay.titleQrwechat;
            },
            ...
            loadQrwechatRender: function () {
                var self = this;
                fullScreenLoader.startLoader();
                self.isPlaceOrderActionAllowed(false);
                $.ajax({
                    type: 'POST',
                    url: window.gbprimepay.beforeQrwechat,
                    dataType: "json",
            ...
            this.isPlaceOrderActionAllowed(false);
            if ($orderId) {
                if (this.item.method == 'gbprimepay_qrwechat') {
setTimeout(function () {
    window.location.replace(url.build('gbprimepay/checkout/pendingqrwechat/id/' + $orderId + '/key/' + $orderKey));
}, 200);
                                }
            }
