...
class ActiveCallbackInstallment extends FormField
{
     ...
     *
     * @return \GBPrimePay\Payments\Block\Adminhtml\System\Config\ActiveCallbackInstallment
     */
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        if ($check) {
            $buttonLabel = "GBPrimePay Credit Card Installment : Active";
            $class = "callback-actived";
        } else {
            $buttonLabel = "GBPrimePay Credit Card Installment : Inactive";
            $class = "";
        }
        ...
        $this->addData(
            [
                ...
                'html_id' => "active_callback_installment_transaction_button",
                'ajax_url' => $this->_urlBuilder->getUrl('gbprimepay/system_config/activeCallbackInstallment'),
            ]
        );
    ...
    public function checkEnable()
    {
        try {
          if ($this->_config->getActiveInstallment()) {
            $callbackUrl = $this->storeManager->getStore()->getBaseUrl() . 'gbprimepay/checkout/callbackinstallment';
