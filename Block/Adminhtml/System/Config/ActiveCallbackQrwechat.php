...
class ActiveCallbackQrwechat extends FormField
{
     ...
     *
     * @return \GBPrimePay\Payments\Block\Adminhtml\System\Config\ActiveCallbackQrwechat
     */
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        if ($check) {
            $buttonLabel = "GBPrimePay QR Wechat : Active";
            $class = "callback-actived";
        } else {
            $buttonLabel = "GBPrimePay QR Wechat : Inactive";
            $class = "";
        }
        ...
        $this->addData(
            [
                ...
                'html_id' => "active_callback_qrwechat_transaction_button",
                'ajax_url' => $this->_urlBuilder->getUrl('gbprimepay/system_config/activeCallbackQrwechat'),
            ]
        );
    ...
    public function checkEnable()
    {
        try {
          if ($this->_config->getActiveQrwechat()) {
            $callbackUrl = $this->storeManager->getStore()->getBaseUrl() . 'gbprimepay/checkout/callbackqrwechat';
