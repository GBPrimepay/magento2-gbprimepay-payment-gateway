...
class ActiveCallbackQrcredit extends FormField
{
     ...
     *
     * @return \GBPrimePay\Payments\Block\Adminhtml\System\Config\ActiveCallbackQrcredit
     */
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        if ($check) {
            $buttonLabel = "GBPrimePay QR Visa : Active";
            $class = "callback-actived";
        } else {
            $buttonLabel = "GBPrimePay QR Visa : Inactive";
            $class = "";
        }
        ...
        $this->addData(
            [
                ...
                'html_id' => "active_callback_qrcredit_transaction_button",
                'ajax_url' => $this->_urlBuilder->getUrl('gbprimepay/system_config/activeCallbackQrcredit'),
            ]
        );
    ...
    public function checkEnable()
    {
        try {
          if ($this->_config->getActiveQrcredit()) {
            $callbackUrl = $this->storeManager->getStore()->getBaseUrl() . 'gbprimepay/checkout/callbackqrcredit';
