...
class ActiveCallbackDirect extends FormField
{
     ...
     *
     * @return \GBPrimePay\Payments\Block\Adminhtml\System\Config\ActiveCallbackDirect
     */
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        if ($check) {
            $buttonLabel = "GBPrimePay Credit Card : Active";
            $class = "callback-actived";
        } else {
            $buttonLabel = "GBPrimePay Credit Card : Inactive";
            $class = "";
        }
        ...
        $this->addData(
            [
                ...
                'html_id' => "active_callback_direct_transaction_button",
                'ajax_url' => $this->_urlBuilder->getUrl('gbprimepay/system_config/activeCallbackDirect'),
            ]
        );
    ...
    public function checkEnable()
    {
        try {
          if ($this->_config->getActiveDirect()) {
            $callbackUrl = $this->storeManager->getStore()->getBaseUrl() . 'gbprimepay/checkout/callbackdirect';
